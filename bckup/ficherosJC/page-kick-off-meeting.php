<?php get_header(); ?>
<style>
@media (max-width: 768px) {
	.section-wrap section{width:100%!important; float:left;}
	.multiaventura .vuela {
	padding:5%;
	}
	.multiaventura .eventos-nav {
 height: auto;
}
	.multiaventura .vuela .section-wrap section:nth-child(2n + 1) {
		margin-left: 0;margin-right: 0;
	}
	.multiaventura .vuela .section-wrap section:nth-child(2n + 2) {
		margin-left: 0;margin-right: 0;
	}
	.head_layout {
		padding: 0;
	}
	.meeting {
	width: 90%;
	}
	.kick-off .eventos-nav {
	 height: auto;
}	
	.kick-off .eventos-nav ul {
		width: 100%;min-width: 0px;
	}
	.kick-off .eventos-nav li {
		width: 100%;margin-top: 20px;
	}

}
.multiaventura .end {
	background: none;
}
.multiaventura .end span {
font-size: 14px;
}
</style>
<div id="container" class="pagina kick-off">	
	 <?php get_template_part( 'include/cabecera', 'paginas' ); ?> 

<div class="fullscreen background parallax" style="background-image:url('<?php echo get_template_directory_uri(); ?>/images/kickoff_atm_1.jpg');" data-img-width="1600" data-img-height="891" data-diff="100">
    <div class="content-a">
        <div class="content-b">
        	<div class="section-wrap first"> 
        		<header>
        			<div class="shadow">
        				
        			</div>
        			<div class="head_layout">
        				<h1>Kick off meeting</h1>
        			</div>
        		</header>
		        
		        <h2>"Pasión por competir ... con uno mismo"</h2>
		          <div class="head_layout">
			          <section>
			           		<p class="p1">Para organizar un encuentro con clientes, empleados o proveedores; una reunión para planificar, reestructurar o alinear objetivos o ideas; un inicio o fin de proyecto, etapa o período… resulta muy interesante hacer <i>un kick-off</i>.</p>
							<p class="p1">Un buen <b>plan de incentivos</b> o de motivación es un modo eficaz de reconocer y valorar el trabajo y los logros de un equipo. Los kick-off resultan ocasiones idóneas para acercar posturas y posicionamientos, recabar información y que la motivación se fortalezca. En Amazonia Team Factory te brindamos la oportunidad de que puedan desarrollar una reunión o meeting y clausurarla con un evento deportivo.</p>
			           </section>
		          </div>
            </div>
        </div>
    </div>
</div>

<div class="mejorate background" style="background-image:url('<?php echo get_template_directory_uri(); ?>/images/kickoff_atm_naranja.png');" data-img-width="1600" data-img-height="358" data-diff="100">
    <div class="content-a">
        <div class="content-b">
        	<div class="section-wrap background second" style="background-image:url('<?php echo get_template_directory_uri(); ?>/images/kickoff_atm_2.jpg');" data-img-width="1600" data-img-height="358" data-diff="100"> 
		        <h2>"mejórate a ti mismo"</h2>
            </div>
         	<div class="head_layout">

         		<div class="meeting">
         			<section>
		          		<h3 class="fondo_blanco">kick-off meeting</h3>
						<p class="p1">En estos eventos se estimula la sana competición o fair-play a lo largo de los periplos de pruebas que les proponemos. Este estímulo, impulsa la motivación por el logro, el espíritu de superación y la conciencia de ventaja competitiva.</p>
						<p class="p1">Los juegos son variados, contamos con toda nuestra infraestructura de aventura en los árboles, juegos de ingenio, de orientación, y supervivencia, hasta modalidades de juegos de <i>Challenge</i> (retos), Construcción, <i>Team Games</i> y<i> Problem Solving</i>.</p>
		           </section>
         			
         		</div>
		          
	        </div>
        </div>
    </div>
</div>

<div class="background not-fullscreen parallax" style="background-image:url('<?php echo get_template_directory_uri(); ?>/images/kickoff_atm_3.jpg');" data-img-width="1600" data-img-height="891" data-diff="100">
    <div class="content-a">
        <div class="content-b">
        	<div class="section-wrap third"> 
		        <h2>"No hay reto más desafiante que el manejarse a si mismo"</h2>
            </div>
        </div>
    </div>
</div>

<div class="fourth">
	<h2>Elige tu evento de empresa</h2>
	<div class="head_layout">

		<div class="eventos-nav">
			<ul>
				<li class="page_item page-item-26"><a href="<?php echo site_url(); ?>/eventos-de-empresa/jornada-multiaventura/"><span>Jornada Multiaventura</span></a></li>
				<li class="page_item page-item-30"><a href="<?php echo site_url(); ?>/eventos-de-empresa/team-event/"><span>Team Event</span></a></li>
				<li class="page_item page-item-32"><a href="<?php echo site_url(); ?>/eventos-de-empresa/gymkana-tematica/"><span>Gymkana Temática</span></a></li>
			</ul>
		</div>
	</div>
</div>

<div class="end">
	<div class="head_layout">
		<h2>¿necesitas más información?</h2>
		<ul class="contacto">
			<a href="<?php echo site_url(); ?>/contactar"></a>
			<a href="<?php echo site_url(); ?>/contactar"></a>
			<a href="<?php echo site_url(); ?>/contactar"></a>
		</ul>
		<span>Contacta con nosotros</span>
	</div>
</div>


<?php get_footer(); ?>